...
/**
 * JasperReports Service for MotorPH Payroll System
 * Implements professional PDF payslip generation using MotorPH template
 */
public class JasperPayslipService {
        ...
        if (templateStream == null) {
            throw new JasperReportException(
                    "MotorPH template not found: " + PAYSLIP_TEMPLATE +
                            "\nPlease ensure motorph_payslip.jrxml is in resources/"
            );
        }
        try {
    ...
    private JasperReport getCompiledReport() throws JRException {
        ...
        if (jrxmlStream == null) {
            throw new JRException("MotorPH template not found: " + PAYSLIP_TEMPLATE);
        }
    ...
    private byte[] exportReport(JasperPrint jasperPrint, ExportFormat format) throws JRException {
        ...
        switch (format) {
            case PDF:
                ...
                pdfConfig.setMetadataSubject("Official Payroll Document");
